...
# 获取限流器实例 (5 QPS for lab summaries)
_lab_limiter = RateLimitConfig.get_limiter("lab_summary", qps=5.0, burst_size=10.0)
...
async def generate_lab_summary(request: LabSummaryRequest):
    ...
    # 1. 限流检查
    if not _lab_limiter.try_acquire():
        wait_time = int(_lab_limiter.get_wait_time() + 1)
        raise HTTPException(
            status_code=429,
...
async def _generate_summary_with_retry(request: LabSummaryRequest) -> str:
    """
    带重试的摘要生成
